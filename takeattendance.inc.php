...
$user = mysqli_query($conn, "SELECT usersId FROM users WHERE usersUid = '$studentName'");
...
function takeabsent($conn,$sId,$cId,$dateOfAbsent){
  ...
  if (!mysqli_stmt_prepare($stmt, $sql)) {
    header("location: ./newTake.php?error=stmtfailed");
    exit();
}
mysqli_stmt_bind_param($stmt, "sss",$sId,$cId,$dateOfAbsent);
mysqli_stmt_execute($stmt);
mysqli_stmt_close($stmt);
header("location: ./newTake.php?error=none");
exit();
}
